...
mean_availability_30 <- aggregate(listings$availability_30, list(listings$city), mean)
barplot(height = mean_availability_30$x, names.arg = mean_availability_30$Group.1)

## Find the average revenue over 30 days of listings per each city
mean_revenue_30 <- aggregate(listings$revenue_30, list(listings$city), mean)
barplot(height = mean_revenue_30$x, names.arg = mean_revenue_30$Group.1)
...
## Comparing the distribution of estimated revenue for the next 30 days of listings
## per each city and for each house size
malaga <- listings[listings$city == "malaga",]
mallorca <- listings[listings$city == "mallorca",]
sevilla <- listings[listings$city == "sevilla",]
p <- ggplot(malaga, aes(bedrooms, revenue_30))
p + geom_boxplot(aes(colour = "red"), outlier.shape = NA) +
    scale_y_continuous(limits = quantile(listings$revenue_30, c(0.1, 0.9), na.rm = T))
p <- ggplot(mallorca, aes(bedrooms, revenue_30))
p + geom_boxplot(aes(colour = "red"), outlier.shape = NA) +
    scale_y_continuous(limits = quantile(listings$revenue_30, c(0.1, 0.9), na.rm = T))
p <- ggplot(sevilla, aes(bedrooms, revenue_30))
p + geom_boxplot(aes(colour = "red"), outlier.shape = NA) +
    scale_y_continuous(limits = quantile(listings$revenue_30, c(0.1, 0.9), na.rm = T))

## Comparing the distribution of estimated revenue for the next 30 days of listings
## per each city and for each house size
p <- ggplot(malaga, aes(room_type, revenue_30))
p + geom_boxplot(aes(colour = "red"), outlier.shape = NA) +
    scale_y_continuous(limits = quantile(listings$revenue_30, c(0.1, 0.9), na.rm = T))
p <- ggplot(mallorca, aes(room_type, revenue_30))
p + geom_boxplot(aes(colour = "red"), outlier.shape = NA) +
    scale_y_continuous(limits = quantile(listings$revenue_30, c(0.1, 0.9), na.rm = T))
p <- ggplot(sevilla, aes(room_type, revenue_30))
p + geom_boxplot(aes(colour = "red"), outlier.shape = NA) +
    scale_y_continuous(limits = quantile(listings$revenue_30, c(0.1, 0.9), na.rm = T))
